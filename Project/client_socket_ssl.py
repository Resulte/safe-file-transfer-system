import socket
import ssl,time,os,struct,json,tkinter,rsa
...
class client_ssl:
    def __init__(self):
        ...
        # 加载客户端所用证书和私钥
        context.load_cert_chain(os.path.dirname(__file__)+'/cer/client/client.crt', os.path.dirname(__file__)+'/cer/client/client_rsa_private.pem')
        # 双向校验模式
    ...
    def download(self, filename):
        ...
        if buf:  # 如果不加这个if，第一个文件传输完成后会自动走到下一句
            ...
            if stat == 'Success':
                ...
                # filenewname = os.path.join(os.path.dirname(__file__)+'/ClientDownload/', filename)
                # print('file new name is %s, filesize is %s' % (filenewname, fileSize))
                # recvd_size = 0  # 定义接收了的文件大小
                # file = open(filenewname, 'wb')
                # print('start receiving...')
